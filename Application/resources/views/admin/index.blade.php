        ...
        $(document).ready(function() {
            ...
            function openModal(type) {
                ...
                if (type === 'topup') {
                    ...
                    modalTitle.text('Tarik Saldo Kas');
    ...
    <script>
        $(document).ready(function() {
            ...
            function resetLayout() {
                ...
                main.removeClass('md:w-1/2').addClass('md:w-full');
            }

            function showCard(cardId) {
                main.removeClass('md:w-full').addClass('md:w-1/2');
            ...
            let type = window.location.pathname.includes('menu') ? 'menu' : 'user';
            ...
            $('#datatables').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: "{{ route('admin_datatable', ':type') }}".replace(':type', type),
                    type: 'GET',
                    dataType: 'json',
                },
            });
            ...
            $(document).on('click', '.btn-view', function(e) {
                ...
                const url = "{{ route('admin_form_view', ['type' => 'TYPE', 'id' => 'ID']) }}"
                    .replace('TYPE', type)
                    .replace('ID', id);
            ...
            $(document).on('click', '.btn-edit', function(e) {
                ...
                const id = $(this).data('id');
                const type = $(this).data('type');
                const url = "{{ route('admin_form_edit', ['type' => 'TYPE', 'id' => 'ID']) }}"
                    .replace('TYPE', type)
                    .replace('ID', id);
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        notyf.error('Gagal memuat form edit');
                    }
                });
            });

            $(document).on('change', '.changerolemenu', function() {
                const checkbox = $(this);
                const roleid = checkbox.val();
                const menuid = checkbox.data('menuid');
                const checked = checkbox.is(':checked');

                const group = $('.changerolemenu[data-menuid="' + menuid + '"]');
                const checkedCount = group.filter(':checked').length;

                if (!checked && checkedCount === 0) {
                    checkbox.prop('checked', true);
                    notyf.error('Minimal pilih 1 role');
                    return;
                }

                if (checked) {
                    group.not(checkbox).prop('checked', false);
                }
                $.ajax({
                    url: "{{ route('admin_changerole') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        roleid: roleid,
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                const id = $(this).data('id');
                const type = $(this).data('type');
                const url = "{{ route('admin_delete', ['type' => 'TYPE', 'id' => 'ID']) }}"
                    .replace('TYPE', type)
                    .replace('ID', id);
            ...
            var selectedRoleText = $('#roleSelect').data('selected-text');
            if (selectedRoleId && selectedRoleText) {
                var roleOption = new Option(selectedRoleText, selectedRoleId, true, true);
                $('#roleSelect').append(roleOption).trigger('change');

                if (selectedRoleId == 2) {
                    $('#namatoko').removeClass('hidden');
                }
            }

            $("#roleSelectMenu").select2({
                placeholder: "Pilih Role",
                ajax: {
                    url: "{{ route('select_roles') }}",
                    dataType: 'json',
            ...
            var selectedRoleMenuId = $('#roleSelectMenu').data('selected');
            var selectedRoleMenuText = $('#roleSelectMenu').data('selected-text');
            if (selectedRoleMenuId && selectedRoleMenuText) {
                var roleMenuOption = new Option(selectedRoleMenuText, selectedRoleMenuId, true, true);
                $('#roleSelectMenu').append(roleMenuOption).trigger('change');
            }
            ...
            $("#submenu").select2({
                ...
                allowClear: true,
                ajax: {
                    url: "{{ route('select_submenus') }}",
                    dataType: 'json',
                    processResults: function(data) {
                        return {
                            results: data.map(function(msmenu) {
                                return {
                                    id: msmenu.menuid,
                                    text: msmenu.menunm
                                };
            ...
            var selectedSubmenuId = $('#submenu').data('selected');
            var selectedSubmenuText = $('#submenu').data('selected-text');
            if (selectedSubmenuId && selectedSubmenuText) {
                var submenuOption = new Option(selectedSubmenuText, selectedSubmenuId, true, true);
                $('#submenu').append(submenuOption).trigger('change');
            }
